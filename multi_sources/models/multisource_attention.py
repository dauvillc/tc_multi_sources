"""Implements the MultisourceAttention class."""
...
class MultisourceAttention(nn.Module):
    ...
    performs a multi-source attention operation.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            num_heads (int): Number of heads in the attention mechanism.
                Must be a divisor of embedding_dim.
        ...
        # Create an embedding for the keys, queries and values
        self.key_embedding = nn.Linear(self.keys_patch_dim, embedding_dim)
        self.values_embedding = nn.Linear(self.values_patch_dim, embedding_dim)
        self.query_embedding = nn.Linear(self.queries_patch_dim, embedding_dim)
        ...
        # Output layer to project the values back to the original size.
        self.output_proj = nn.Linear(embedding_dim, self.values_patch_dim)
    ...
    def forward(self, keys, values, queries):
        ...
        queries_emb = self.query_embedding(queries_seq)  # (b, hd, n_queries_seq, emb_dim)
        # Reshape to add the heads dimension.
        keys_emb = self.to_heads(keys_emb)  # (b, hd, n_keys_seq, emb_dim)
        values_emb = self.to_heads(values_emb)  # (b, hd, n_keys_seq, emb_dim)
        queries_emb = self.to_heads(queries_emb)  # (b, hd, n_queries_seq, emb_dim)
        # Compute the attention weights.
        attn = torch.matmul(queries_emb, keys_emb.transpose(-2, -1))
        attn = F.softmax(attn / (self.dim_head**0.5), dim=-1)  # (b, hd, n_queries_seq, n_keys_seq)
        # Compute the output sequence.
        out_seq = torch.matmul(attn, values_emb)  # (b, hd, n_queries_seq, emb_dim)
        # Remove the heads dimension.
        out_seq = rearrange(out_seq, "b hd nq d -> b nq (hd d)")
        # Project the output to the original values patch dimension.
        out_seq = self.output_proj(out_seq)  # (b, n_queries_seq, v_patch_dim)
        # Split the sequence by query source.
        outs = torch.split(out_seq, n_queries_patches, dim=1)
        # Reshape each output to the size of the corresponding query image
        outs = [
            rearrange(
                out,
            ...
            for out, qs in zip(outs, query_sizes)
        ]
        # Remove the padding from the outputs.
        outs = [out[..., : os[0], : os[1]] for out, os in zip(outs, original_query_sizes)]
        return outs


class MultisourceAttBlock(nn.Module):
    """Wrapper class that integrates the Multi-source Attention module
