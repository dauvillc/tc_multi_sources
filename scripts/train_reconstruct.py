...
def main(cfg: DictConfig):
    ...
    input_sources = [
        source for source in train_dataset.get_source_names() if source not in masked_sources
    ]
    n_masked_sources = len(masked_sources)
    n_input_sources = len(input_sources)
    input_sources_channels = [train_dataset.get_n_variables(source) for source in input_sources]
    masked_sources_channels = [train_dataset.get_n_variables(source) for source in masked_sources]
    model = instantiate(
        cfg["model"],
        n_input_sources,
        n_masked_sources,
